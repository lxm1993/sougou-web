...
define(['jquery', 'kmap', 'underscore', 'bootbox', 'bootstrap', 'bootstrapTable', 'bootstrapTableZh'], function($, kmap, _, bootbox){
    var table = {};
    table.init = function(){
        kmap.buildMenu('task');
    ...
    var template = _.template('<table class="parser-table"><tr><th>ID</th><th>URL</th><th>表ID&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th><th>创建者</th><th>版本</th></tr>' +
        '<% _.each(parsers, function(item){ %>' +
        '<tr><td> <%= item.id %></td>' +
        '<td class="parser-td"><a href="/parser/edit-parser?url=<%= encodeURIComponent(item.parserUrl) %>"><%= item.parserUrl %></a>' +
        '&nbsp;&nbsp;<a title="调试" href="javascript:;" class="link-green parser-debug"><span class="glyphicon glyphicon-expand" style="top:3px"></span></a></td>' +
        '<td><a href="/data/list-instance-data?tableId=<%= item.tableId %>">d9fc10fb-5b6e-46de-9ef8-17af0a135321</a></td>' +
        '<td> <%= item.creator %></td>' +
        '<td> <%= item.latestVersion %></td></tr>' +
        '<% }) %>' +
        '</table>');
    function parserFormatter(value, row){
        if(value && typeof value === 'string'){
            ...
            //var hlist = ['<ul class="parser-ul">'];
            //for(var i = 0; i < value.length; i++){
            //    var item = value[i];
            //    hlist.push('<li><a href="/parser/list-parser?parser=' + encodeURIComponent(row.seeds) + '">' + item.parserUrl + '</a><span>&nbsp;&nbsp;&nbsp;' + item.latestVersion + '</span></li>');
            //}
            //hlist.push('</ul');
            //return hlist.join('');
        }else{
    ...
    function operateFormatter(value, row, index){
        var name = row.taskId || '';
        var result =  ['<a title="修改" href="javascript:;" class="link-blue"><span class="glyphicon glyphicon-pencil"></span></a>',
            '&nbsp;&nbsp;&nbsp;',
            '<a title="删除" href="javascript:;" class="link-red"><span class="glyphicon glyphicon-remove"></span></a>',
            '&nbsp;&nbsp;&nbsp;',
            '<a title="运行" href="javascript:;" class="link-green startTask" name="',
            name,
            '"><span class="glyphicon glyphicon-play"></span></a>',
            '&nbsp;&nbsp;&nbsp;',
            '<a title="停止" href="javascript:;" class="link-red" name="',
            name,
