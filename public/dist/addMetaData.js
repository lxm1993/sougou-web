...
/***/ function(module, exports, __webpack_require__) {
	...
	!function ($, _) {
	    ...
	    SchemaTable.templates = function () {
	        ...
	        var root = _.template('<tr data-schema-id="root" class="schema-tr schema-expanded"><td>' + indentIcon + '<div class="schema-root">所有列</div></td><td>&nbsp;</td><% if(isAdd){ %><td>' + addIcon + '</td><% } %></tr>');
	        var tr = _.template('<tr data-schema-id="<%= schemaId %>" data-schema-parent-id="<%= parentId %>" class="schema-tr">' + '<td><% for(var i = 0; i <= depth; i++){ %>' + indentIcon + '<% } %>' + '<div class="schema-form-control"><input value="<%= schema.name %>" type="text" placeholder="请输入列名称" class="form-control input-sm schema-name"></div></td>' + '<td><div class="form-group"><select class="form-control input-sm schema-type">' + '<option value="0" <%if(schema.type === 0){%> selected="selected" <%}%> >数值</option> ' + '<option value="1" <%if(schema.type === 1){%> selected="selected" <%}%> >字符串</option>' + '<option value="2" <%if(schema.type === 2){%> selected="selected" <%}%> >日期</option> ' + '<option value="3" <%if(schema.type === 3){%> selected="selected" <%}%> >坐标</option> ' + '<option value="4" <%if(schema.type === 4){%> selected="selected" <%}%> >对象</option> </select></div></td>' + '<td><%if(schema.type == 4){%> ' + addIcon + ' <%}%>' + deleteIcon + '</td></tr>');
	        var tr2 = _.template('<tr data-schema-id="<%= schemaId %>" data-schema-parent-id="<%= parentId %>" class="schema-tr">' + '<td><% for(var i = 0; i <= depth; i++){ %>' + indentIcon + '<% } %>' + '<div class="schema-form-control"><input value="<%= schema.name %>" type="text" placeholder="请输入列名称" class="form-control input-sm schema-name"></div></td>' + '<td><div class="form-group"><select disabled="disabled" class="form-control input-sm schema-type">' + '<option value="0" <%if(schema.type === 0){%> selected="selected" <%}%> >数值</option> ' + '<option value="1" <%if(schema.type === 1){%> selected="selected" <%}%> >字符串</option>' + '<option value="2" <%if(schema.type === 2){%> selected="selected" <%}%> >日期</option> ' + '<option value="3" <%if(schema.type === 3){%> selected="selected" <%}%> >坐标</option> ' + '<option value="4" <%if(schema.type === 4){%> selected="selected" <%}%> >对象</option> </select></div></td>' + '</tr>');
	        return {
