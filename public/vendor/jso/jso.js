...
define('jso',['require','exports','module','./store','./utils','./Config'],function(require, exports, module) {
	...
	JSO.prototype.callback = function(url, callback, providerID) {
        ...
        var cookieExpires = now + 3600*24*365*100;
        if(atoken.expires){
            cookieExpires = atoken.expires;
        }
        document.cookie = 'access_token=' + encodeURIComponent(atoken.access_token)
            + '; expires='+ (new Date(cookieExpires * 1000)).toGMTString() + '; path=/';


        if (window.history.replaceState) {
            if (state.restoreHash) {
                var storeHash = state.restoreHash;
                if (storeHash.charAt(0) !== '#') {
                    storeHash = '#' + storeHash;
                }
                window.history.replaceState('jso', 'jso', storeHash);
            } else {
                window.history.replaceState('jso', 'jso', '#');
            }
        } else {
            if (state.restoreHash) {
